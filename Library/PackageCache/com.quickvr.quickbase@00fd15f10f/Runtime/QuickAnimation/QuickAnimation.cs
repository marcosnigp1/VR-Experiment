...
namespace QuickVR
{
    ...
    public class QuickAnimation
    {
        ...
        public virtual void AddKey(Animator animator, float time, bool forceAdd = false)
        {
            AddKey(animator, null, time, forceAdd);
        }

        public virtual void AddKey(Animator animator, SkinnedMeshRenderer[] smRenderers, float time, bool forceAdd = false)
        {
            QuickAnimationKeyframe kFrame = new QuickAnimationKeyframe();
            kFrame._time = time;

            AddKey(CURVE_TRANSFORM_POSITION, time, animator.transform.position, forceAdd, kFrame);
            AddKey(CURVE_TRANSFORM_ROTATION, time, animator.transform.rotation, forceAdd, kFrame);

            if (animator.isHuman)
            {
                QuickHumanPoseHandler.GetHumanPose(animator, ref _pose);
                ...
                animator.GetIKGoalFromBodyPose(AvatarIKGoal.LeftFoot, bodyPosition, bodyRotation, out ikGoalPos, out ikGoalRot);
                ...
                animator.GetIKGoalFromBodyPose(AvatarIKGoal.RightFoot, bodyPosition, bodyRotation, out ikGoalPos, out ikGoalRot);
                ...
                {
                    SkinnedMeshRenderer r = smRenderers[0];
                    for (int i = 0; i < r.sharedMesh.blendShapeCount; i++)
                    {
                        string bsName = "bs" + i.ToString();
                        float w = r.GetBlendShapeWeight(i);
                        AddKey(bsName, time, w, forceAdd, kFrame);
                    }
                }
            }
        ...
        public virtual void AddKeyFrame(QuickAnimationKeyframe kFrame)
        {
            _keyFrames.Add(kFrame);

            if (kFrame._time > _timeLength)
            {
                _timeLength = kFrame._time;
            }
        }
        ...
        public virtual void EvaluateHumanPose(Animator animator, float time, ref HumanPose pose)
        {
            if (animator.isHuman)
            {
                //Evaluate the body pose
        ...
        public virtual void EvaluateBlendShapes(SkinnedMeshRenderer[] smRenderers, float time)
        {
            if (smRenderers != null && smRenderers.Length > 0)
            {
                foreach (SkinnedMeshRenderer r in smRenderers)
                {
                    ...
                    {
                        string bsName = "bs" + i.ToString();
                        QuickAnimationCurve bsCurve = GetAnimationCurve(bsName);
                        float w = Mathf.Clamp(bsCurve.Evaluate(time), 0, 100);
