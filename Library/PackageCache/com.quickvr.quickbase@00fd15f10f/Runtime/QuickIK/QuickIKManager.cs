...
namespace QuickVR
{
    ...
    public class QuickIKManager : QuickBaseTrackingManager
    {
        ...
        protected virtual void CreateIKSolvers()
        {
            ...
            _animator.EnforceTPose();   //It is necessary the Avatar to be in TPose for the orientation assumption of the bones that we do at the beginning. 
            ...
            GetIKSolver(HumanBodyBones.Head)._targetHint.parent = _ikTargetsRoot;
        ...
        protected virtual void ResetIKTarget(HumanBodyBones boneID, Transform ikTarget)
        {
            if ((int)boneID != -1)
            {
                ...
                if (boneID == HumanBodyBones.LeftHand || boneID == HumanBodyBones.RightHand)
                {
                    ...
                    int sign = isLeft ? -1 : 1;
                    ikTarget.LookAt(ikTarget.position + sign * transform.right, transform.up);
                }
                else if (boneID == HumanBodyBones.Spine || boneID == HumanBodyBones.Chest)
                {
                ...
                {
                    ikTarget.parent = _ikTargetsRoot;
        ...
        protected virtual Transform GetIKTargetParent(HumanBodyBones boneID)
        {
            ...
            {
                return GetIKSolver(HumanBodyBones.Head)._targetLimb;
            }
            if (QuickHumanTrait.IsBoneFingerLeft(boneID))
            {
                return GetIKSolver(HumanBodyBones.LeftHand)._targetLimb;
            }
            if (QuickHumanTrait.IsBoneFingerRight(boneID))
            {
                return GetIKSolver(HumanBodyBones.RightHand)._targetLimb;
            }

            return _ikTargetsRoot;
        }

        public virtual void RestoreIKTargetParent(HumanBodyBones boneID)
        {
            GetIKSolver(boneID)._targetLimb.parent = GetIKTargetParent(boneID);
        }
        ...
        protected virtual HumanBodyBones GetBoneMidID(HumanBodyBones boneLimbID)
        {
            ...
            {
                boneMidID = HumanBodyBones.Chest;
            }
            else if (boneLimbID == HumanBodyBones.LeftEye || boneLimbID == HumanBodyBones.RightEye)
            {
